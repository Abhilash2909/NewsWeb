...
const Navbar = ({ setCategory, setCountry }) => {
  ...
  const handleCountryChange = (event) => {
    ...
    setCountry(event.target.value); // Pass selected country to parent component
  };

  return (
    <nav className={`navbar navbar-expand-lg bg-body-tertiary ${darkMode ? 'dark-mode' : ''}`} data-bs-theme={darkMode ? 'light' : 'dark'}>
      <div className="container-fluid">
        <a className="navbar-brand" href="#">
          <span className="badge bg-light text-dark fs-6">NewsWeb</span>
        </a>
        ...
        <div className="collapse navbar-collapse" id="navbarNav">
          <ul className="navbar-nav me-auto">
              ...
              <ul className="dropdown-menu" aria-labelledby="countryDropdown">
                {countries.map((country) => (
                  <li key={country.value}>
                    <a className="dropdown-item" href="#" onClick={() => handleCountryChange(country.value)}>
                      {country.label}
                    </a>
                  </li>
                ))}
            ...
            {/* ... rest of your navbar categories */}
          </ul>
          <form className="d-flex ms-auto">
            <button className="btn btn-dark ms-2" type="button" onClick={handleDarkModeToggle}>
              {darkMode ? 'Dark Mode' : 'Light Mode'}
